...
def load_processed_energy_data():
    ...
    try:
        ...
        register_cols = []
        for i in range(1, 10):
            matched_v = next((c for c in df.columns if c.upper() == f"V{i}" or c.upper().startswith(f"V{i} ")), None)
            if matched_v:
...
with tab_energy:
    if e_df is not None and not e_df.empty:
        ...
        with c1: 
            st.metric("Dunkin' Total", f"{get_sum(dunkin_col):,.1f} kWh", delta=f"Avg: {get_avg(dunkin_col):,.1f} kWh/day")
        with c2: 
            st.metric("CLC Total", f"{get_sum(clc_col):,.1f} kWh", delta=f"Avg: {get_avg(clc_col):,.1f} kWh/day")
        with c3: 
            st.metric("BMC Total", f"{get_sum(bmc_col):,.1f} kWh", delta=f"Avg: {get_avg(bmc_col):,.1f} kWh/day")
        with c4: 
            st.metric("Deep Freezer Total", f"{get_sum(deep_col):,.1f} kWh", delta=f"Avg: {get_avg(deep_col):,.1f} kWh/day")
        with c5:
        ...
        for col in eq_cols:
            ...
            fig_zone.add_trace(go.Bar(
                x=x_dates, y=e_df[col].tolist(), name=display_name, marker_color=color,
        ...
        if not diff_energy.empty:
            ...
            for i, col in enumerate(diff_cols):
                fig_delta.add_trace(go.Bar(
                    x=diff_energy['ChartDate'].tolist(), y=diff_energy[col].tolist(),
                    name=col.replace(' Δ', ''), marker_color=delta_colors[i % len(delta_colors)],
                    opacity=0.8, hovertemplate=f'{col}<br>Date: %{{x}}<br>Δ: %{{y:+,.2f}} kWh<extra></extra>'
                ))
        ...
        for col in eq_cols:
            series = e_df[col]
            summary_data.append({
                "Zone": zone_labels.get(col, col), "Total (kWh)": f"{series.sum():,.2f}",
                "Mean (kWh/day)": f"{series.mean():,.2f}", "Min (kWh)": f"{series.min():,.2f}",
                "Max (kWh)": f"{series.max():,.2f}", "Std Dev": f"{series.std():,.2f}",
        ...
        with st.expander("📂 View Pre-Processed Active Energy File Data Table", expanded=False):
            ...
            st.download_button(
                label="📥 Download Active Energy Data as CSV", data=e_df.to_csv(index=False).encode('utf-8'),
                file_name=f"active_energy_{start_date.strftime('%Y%m%d')}_to_{end_date.strftime('%Y%m%d')}.csv",
                mime="text/csv", key="btn_download_energy"
            )
    else:
...
with tab_temp:
    if temp_df is not None and not temp_df.empty:
        ...
        with sc1:
            st.markdown(f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:6px; padding:10px 16px; border-left:4px solid #002D62;"><div style="font-size:9px; font-weight:700; color:#94A3B8; text-transform:uppercase; letter-spacing:0.5px;">Dough 1 Delta Variance Sum</div><div style="font-size:16px; font-weight:800; color:#002D62; margin-top:2px;">{temp_df["consump. dough1"].sum():,.2f} °C</div></div>', unsafe_allow_html=True)
        with sc2:
            st.markdown(f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:6px; padding:10px 16px; border-left:4px solid #0EA5E9;"><div style="font-size:9px; font-weight:700; color:#94A3B8; text-transform:uppercase; letter-spacing:0.5px;">Dough 2 Delta Variance Sum</div><div style="font-size:16px; font-weight:800; color:#002D62; margin-top:2px;">{temp_df["consump. dough2"].sum():,.2f} °C</div></div>', unsafe_allow_html=True)
        with sc3:
            st.markdown(f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:6px; padding:10px 16px; border-left:4px solid #E01934;"><div style="font-size:9px; font-weight:700; color:#94A3B8; text-transform:uppercase; letter-spacing:0.5px;">Perishable Delta Variance Sum</div><div style="font-size:16px; font-weight:800; color:#002D62; margin-top:2px;">{temp_df["consump. perishable"].sum():,.2f} °C</div></div>', unsafe_allow_html=True)
        ...
        with st.expander("📂 View & Download Compiled Temperature Log File Data with Delta Metrics", expanded=False):
            ...
            st.download_button(label="Download Compiled Temperature Data as CSV", data=temp_df.to_csv(index=False).encode('utf-8'), file_name="compiled_temperature_logs.csv", mime="text/csv", key="btn_download_temp")
    else:
...
with tab_power:
    if e_df is not None and not e_df.empty:
        ...
        <style>
        .fac-card {
            background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%); border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px;
            box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03); margin-bottom: 20px; border-left: 5px solid #002D62; transition: transform 0.2s ease;
        }
        .fac-card:hover { transform: translateY(-2px); box-shadow: 0 10px 15px -3px rgba(0,0,0,0.08); }
        .fac-title { font-size: 18px; font-weight: 800; color: #002D62; margin-bottom: 15px; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; }
        .fac-metric { display: flex; justify-content: space-between; margin-bottom: 10px; font-size: 13px; padding: 4px 0; }
        .fac-metric-label { color: #64748b; font-weight: 600; text-transform: uppercase; font-size: 11px; }
        ...
        for idx, fac in enumerate(facilities.keys()):
            with cols[idx]:
                ...
                tot = m.get('total_power', 0)
                st.markdown(f"""
                <div class="fac-card" style="border-left-color: {colors[fac]};">
                    <div class="fac-title">{fac}</div>
                    <div class="fac-metric"><span class="fac-metric-label">Total Power</span><span class="fac-metric-value">{tot:,.0f} kWh</span></div>
                    <div class="fac-metric"><span class="fac-metric-label">Avg Daily</span><span class="fac-metric-value">{m.get('avg_daily', 0):,.1f} kWh</span></div>
                    <div class="fac-metric"><span class="fac-metric-label">Highest Daily</span><span class="fac-metric-value">{m.get('highest_daily', 0):,.1f} kWh</span></div>
                    <div class="fac-metric"><span class="fac-metric-label">Lowest Daily</span><span class="fac-metric-value">{m.get('lowest_daily', 0):,.1f} kWh</span></div>
                    <div class="fac-metric"><span class="fac-metric-label">Total Units</span><span class="fac-metric-value">{tot:,.0f} kWh</span></div>
                    <div class="fac-metric"><span class="fac-metric-label">Days Processed</span><span class="fac-metric-value">{int(m.get('days_processed', 0))}</span></div>
                </div>
                """, unsafe_allow_html=True)
    else:
...
with tab_runtime:
    ...
    if runtime_df is not None and not runtime_df.empty:
        ...
        if kwh_cols and not r.empty:
            ...
            daily_runtime = daily_runtime.rename(columns={'Date_Key': 'Date', 'sum': 'Energy Drew (kWh)', 'max': 'Peak System Load Vector (kWh)', 'mean': 'Mean Load Vector (kWh)'})
            ...
            with rc1:
                st.markdown(f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:6px; padding:10px 16px; border-left:4px solid #002D62;"><div style="font-size:9px; font-weight:700; color:#94A3B8; text-transform:uppercase; letter-spacing:0.5px;">Energy Drew</div><div style="font-size:16px; font-weight:800; color:#002D62; margin-top:2px;">{(target_day["Energy Drew (kWh)"] if target_day is not None else 0):,.1f} kWh</div></div>', unsafe_allow_html=True)
            with rc2:
                st.markdown(f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:6px; padding:10px 16px; border-left:4px solid #FF9F1C;"><div style="font-size:9px; font-weight:700; color:#94A3B8; text-transform:uppercase; letter-spacing:0.5px;">Peak System Load Vector</div><div style="font-size:16px; font-weight:800; color:#002D62; margin-top:2px;">{(target_day["Peak System Load Vector (kWh)"] if target_day is not None else 0):,.1f} kWh</div></div>', unsafe_allow_html=True)
            with rc3:
                st.markdown(f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:6px; padding:10px 16px; border-left:4px solid #E01934;"><div style="font-size:9px; font-weight:700; color:#94A3B8; text-transform:uppercase; letter-spacing:0.5px;">Mean Load Vector</div><div style="font-size:16px; font-weight:800; color:#002D62; margin-top:2px;">{(target_day["Mean Load Vector (kWh)"] if target_day is not None else 0):,.1f} kWh</div></div>', unsafe_allow_html=True)
            ...
            with st.expander("📂 View & Download Asset Duty Cycle Raw Sheet Data", expanded=False):
                ...
                st.download_button(label="Download Date-Wise Duty Cycles as CSV", data=daily_runtime.to_csv(index=False).encode('utf-8'), file_name="datewise_asset_duty_cycles.csv", mime="text/csv", key="btn_download_runtime")
    else:
...
with tab_comp:
    ...
    if comp_raw is not None and not comp_raw.empty:
        ...
        c_df = c_df[(c_df['Parsed_Date'] >= TARGET_START) & (c_df['Parsed_Date'] <= TARGET_END)].copy()
        ...
        if c_df.empty:
            ...
            if len(compressor_config) < 5 and len(c_df.columns) >= 11:
                ...
                for i in range(1, 6):
                    comp_name = f"Compressor-{i}"
                    compressor_config[comp_name] = {'stop': c_df.columns[2 * i - 1], 'start': c_df.columns[2 * i]}
            ...
            if not compressor_config:
                ...
                for comp_name, cols in compressor_config.items():
                    ...
                    for target_date in all_dates:
                        ...
                        daily_records.append({
                            'Date': target_date, 'Compressor': comp_name, 'Working Hours': round(runtime_hrs, 2),
                            'Non Working Hours': round(downtime_hrs, 2), 'Utilization %': round((runtime_hrs / 24.0) * 100.0, 1)
                        })
                    
                    total_available_hrs = total_days * 24.0
                    summary_records.append({
                        'Compressor': comp_name, 'Working Hours': round(total_runtime_hrs, 2), 'Non Working Hours': round(total_downtime_hrs, 2),
                        'Utilization %': round((total_runtime_hrs / total_available_hrs) * 100.0, 1), 'Downtime %': round((total_downtime_hrs / total_available_hrs) * 100.0, 1)
                    })
                ...
                with k1: st.metric("Avg Utilization", f"{df_summary['Utilization %'].mean():.1f}%")
                with k2: st.metric("Avg Downtime", f"{df_summary['Downtime %'].mean():.1f}%")
                with k3: st.metric("Best Performer", df_summary.loc[df_summary['Utilization %'].idxmax(), 'Compressor'])
                with k4: st.metric("Highest Downtime", df_summary.loc[df_summary['Downtime %'].idxmax(), 'Compressor'])
                ...
                with st.expander("📂 Download Processed Compressor Data", expanded=False):
                    ...
                    with col_dl1:
                        st.download_button(label="📥 Download Daily Data (CSV)", data=df_daily.to_csv(index=False).encode('utf-8'), file_name=f"compressor_daily_{TARGET_START.strftime('%Y%m%d')}_to_{TARGET_END.strftime('%Y%m%d')}.csv", mime="text/csv", key="btn_download_comp_daily")
                    with col_dl2:
                        st.download_button(label="📥 Download Summary Data (CSV)", data=df_summary.to_csv(index=False).encode('utf-8'), file_name=f"compressor_summary_{TARGET_START.strftime('%Y%m%d')}_to_{TARGET_END.strftime('%Y%m%d')}.csv", mime="text/csv", key="btn_download_comp_summary")
