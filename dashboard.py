        ...
        if raw_comp_df is None or raw_comp_df.empty:
            ...
            for comp_name, idx_config in compressor_map.items():
                ...
                for i in range(len(c)):
                    ...
                    # INVERSION FIX: If cell values are empty, the compressor was shut down (0 Working Hours)
                    if raw_stop in ['nan', '', 'None', 'NAT', '0', '0.0'] or pd.isna(c.iloc[i, idx_config['stop_idx']]):
                        ...
                        try:
                            ...
                            # Shift operational timeframe runtime calculation logic
                            if t_start >= t_stop:
                                working_hours = (t_start - t_stop).total_seconds() / 3600.0
                            else:
                                # Overnight shift crossover duration correction
            ...
            if analytics_df.empty:
                ...
                # Group data models across metrics scopes
                ...
                st.markdown(f"### 📅 Filtered Window Focus: `{start_window.strftime('%d-%b-%Y')}` to `{end_window.strftime('%d-%b-%Y')}`")
                ...
                with v_col2:
                    ...
                    # 4. Downtime Analysis (Top compressors by downtime descending)
