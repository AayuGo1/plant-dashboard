...
def load_processed_energy_data():
    ...
    try:
        ...
        # Calculate Consumption from Registers
...
# ─────────────────────────────────────────────────────────────
#  EXCEL SHEET LOADER - FIXED NAMEERROR
# ─────────────────────────────────────────────────────────────
@st.cache_data(ttl=300)
def load_excel_sheet(sheet_name, fallback_header_row):
    all_files = list_github_files()  # ← DEFINED HERE TO FIX NAMEERROR
...
with tab_energy:
    if e_df is not None and not e_df.empty:
        ...
        if len(missing_dates) > 0:
            st.markdown(f'<div class="alert-warn">️ <strong>Data Quality Alert:</strong> {len(missing_dates)} missing date(s) detected.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="alert-ok">✓ <strong>Data Integrity:</strong> Complete date coverage.</div>', unsafe_allow_html=True)
        ...
        if existing_v_channels:
            ...
            meter_names = {f'V{i}_Consumption': f'V{i}' for i in range(1,10)}
            
            for i, col in enumerate(existing_v_channels):
                fig.add_trace(go.Scatter(x=x_dates, y=e_df[col].tolist(), mode='lines+markers', name=meter_names.get(col, col), line=dict(width=2.5, color=colors[i % len(colors)])))
                
            fig.update_layout(hovermode="x unified", height=450, xaxis=dict(title='Date', type='category', tickangle=45), yaxis=dict(title='Daily Consumption (kWh)'), legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ...
        for col in eq_cols:
            fig_zone.add_trace(go.Bar(x=x_dates, y=e_df[col].tolist(), name=col.replace(' Consumption',''), marker_color=zone_colors.get(col,'#64748B')))
        
        fig_zone.update_layout(barmode='stack', hovermode="x unified", height=450, xaxis=dict(title='Date', type='category', tickangle=45), yaxis=dict(title='Total Energy (kWh)'), legend=dict(orientation="h", yanchor="bottom", y=1.02))
        st.plotly_chart(fig_zone, use_container_width=True)
        
        # Day-over-Day Section (CLEANED UP - NO DUPLICATES)
        ...
        if not diff_energy.empty:
            ...
            def render_delta_metric(container, col_name, color, label):
                if col_name in e_df_valid.columns:
                    ...
                    delta_val = target_energy_row[delta_key] if delta_key in diff_energy.columns else 0
            ...
            for i, col in enumerate(diff_cols):
                fig_delta.add_trace(go.Bar(x=diff_energy['ChartDate'].tolist(), y=diff_energy[col].tolist(), name=col.replace(' Δ',''), marker_color=delta_colors[i % len(delta_colors)], opacity=0.8))
            
            fig_delta.update_layout(barmode='group', hovermode="x unified", height=400, xaxis=dict(title='Date', type='category', tickangle=45), yaxis=dict(title='Daily Change (kWh)'), legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ...
        for col in eq_cols:
            ...
            anomalies = e_df[series > threshold_upper]
            
            if len(anomalies) > 0:
                st.markdown(f'<div class="alert-warn"><strong>{zone_labels.get(col, col)}:</strong> {len(anomalies)} anomaly day(s) detected (> +2σ)</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="alert-ok"><strong>{zone_labels.get(col, col)}:</strong> Stable consumption pattern</div>', unsafe_allow_html=True)
        ...
        with st.expander("📂 View Pre-Processed Active Energy File Data Table", expanded=False):
            ...
            st.download_button(label=" Download Active Energy Data as CSV", data=csv_data, file_name=f"active_energy_{start_date.strftime('%Y%m%d')}_to_{end_date.strftime('%Y%m%d')}.csv", mime="text/csv")

    else:
        st.markdown('<div class="alert-info"><strong>⚠️ No active energy data captured.</strong></div>', unsafe_allow_html=True)
...
with tab_temp:
    if temp_df is not None and not temp_df.empty:
        ...
        for col in sensors:
            ...
            rows.append({"Asset Node": labels[col], "Total Logs": len(s), "Mean Temp": s.mean(), "Stability (σ)": s.std(), "Compliance Index": (len(s) - exc) / len(s)})
    ...
    if power_df is not None and not power_df.empty:
        ...
        if dunkin_col and clc_col:
            ...
            if not p.empty:
                ...
                with c4:
                    if savings_col: st.metric("Optimized Value Catch", f"₹ {p[savings_col].sum():,.2f}")
                ...
                st.info("No valid power data after filtering outliers.")
        else:
...
with tab_runtime:
    ...
    if runtime_df is not None and not runtime_df.empty:
        ...
        if kwh_cols and not r.empty:
            ...
            st.info("No KWH data found in Sheet2.")
    else:
...
with tab_comp:
    ...
    if comp_df is not None and not comp_df.empty:
        ...
        if sav_col:
            ...
            st.info("No 'saving' column found in Sheet3.")
    else:
